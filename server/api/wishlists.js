...
router.post('/me', requireToken, async(req,res,next) => {
  try {
    ...
    const singleList = await Wishlist.create({...req.body,  userId: req.user.id, receiver: 'me'});
    res.json(singleList);
  } catch (err) {
    next(err);
  }
...
router.post('/others', requireToken, async(req,res,next) => {
  try {
    // make a list for me by default
    // make a list for someone else
    const singleList = await Wishlist.create({...req.body, userId: req.user.id, receiver: 'others'});
    res.json(singleList);
  } catch (err) {
    next(err);
  }
